"""
Mask R-CNN
Train on the Cracks dataset and implement detection model.

Licensed under the MIT License (see LICENSE for details)
Written by Cesar Portocarrero
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 cracks.py train --dataset=/path/to/cracks/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 cracks.py train --dataset=/path/to/cracks/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 cracks.py train --dataset=/path/to/cracks/dataset --weights=imagenet
"""
...
class CracksConfig(Config):
    """Configuration for training on the cracks dataset.
...
class CracksDataset(utils.Dataset):
    ...
    def load_cracks(self, dataset_dir, subset):
        """Load a subset of the Cracks dataset.
        ...
        # Load annotations
        # Labelme Image Annotator v = 4.6.0      
        ...
        for filename in filenames:
            if filename.endswith(".json"):
                ...
                # Ensure that this picture is in this dataset
                imagename = annotation['imagePath']
                if not os.path.isfile(os.path.join(dataset_dir,imagename)):
                    continue
                if len(annotation["shapes"]) == 0:
                    continue
                # If the image is not in the dataset or if there are no valid shapes, the json ("annotation") is not appended
                annotations.append(annotation)
        
        #Printing statement with the number of json files and valid json files
        ...
        # Typically, after labelme's annotation, all same class items have a same name
        # This forces us to annotate as "ball" all annotations labeled "ball"
        # not "ball_1" "ball_2" ...
        labelslist = []
        for annotation in annotations:
            # Get the x, y coordinates of points of the polygons that make up
            # the outline of each object instance. These are stored in the
            # key "shapes" (see json format above)
            shapes = [] 
            classids = []
        ...
        #Print statement about number of class items, should be 1
    ...
    def load_mask(self, image_id):
        ...
        # Convert shape data to a bitmap mask
        # [height, width, instance_count]
        info = self.image_info[image_id]
        mask = np.zeros([info["height"], info["width"], len(info["shapes"])],
                        dtype=np.uint8)
        for idx, points in enumerate(info["polygons"]):
